...
public static class AssetEndpoints
{
    ...
    static async Task<Results<Ok<AssetDetailDto>, ForbidHttpResult, BadRequest<string>>> Create(
        ...
        // IBarcodeRenderer is injected to warm the singleton on the create path —
        // not used directly. Param is named `_barcode` (not `_`) because the
        // method body uses `_ = SendAssetAssignedEmail(...)` as a true discard.
        // Naming the parameter `_` would shadow the discard and trigger
        // CS0266: Cannot convert Task to IBarcodeRenderer.
        IBarcodeRenderer _barcode, IAuditLogger audit, IEmailSender email, CancellationToken ct)
    {
